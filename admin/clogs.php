...
$me =  User::get_username($session->user_id);
  ...
  if(isset($_POST['clear'])) {
	  ...
	  clog_action('Logs Cleared', "by User ID {$session->user_id} | {$me}");
    // redirect to this same page so that the URL won't 
    // have "clear=true" anymore
    redirect_to('clogs.php');
  }
...
?>
    <div class="admin_left_content">
        	...
        	<div class="feat_prod_box">
                    ...
                    <div class="prod_title">Customer Logs :</div>
                    <p class="details">Following are the details about Customers that logged from Front-End or clear the Logs File : </p>
                    
                    <?php
                        if( file_exists($logfile) && is_readable($logfile) && $handle = fopen($logfile, 'r')) 
                        {  // read
                             echo "<ul style=\"margin-left: 50px; padding-bottom: 20px;\"><br/>";
		                     while(!feof($handle)) {
			                     $entry = fgets($handle);
			                     if(trim($entry) != "") {
            ...
            <form action="clogs.php" method="post">
            ...
            {    
                ...
                <a href="clogs.php" style="font: xx-small;" class="more" ><input type="clr" name="Reload" id="Reload"  value="Reload" class="register" /></a><br />
            </form>
            </div>
            </div>	
